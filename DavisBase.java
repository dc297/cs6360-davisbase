...
public class DavisBase {
	...
	static String prompt = Constants.PROMPT;
	static String dir_catalog = Constants.dirCatalog;
	static String dir_userdata = Constants.dirUserdata;
...
//		System.out.println(copyright);
	...
	public static boolean tableExists(String tableName){
		...
		try {
			...
			File data_dir = new File(dir_userdata);
...
public static void initialize() {
		...
		try {
			RandomAccessFile catalogTable = new RandomAccessFile(dir_catalog+"/davisbase_tables.tbl", "rw");
		...
		try {
			RandomAccessFile catalogColumn = new RandomAccessFile(dir_catalog+"/davisbase_columns.tbl", "rw");
			...
			Table.insertInto(Constants.COLUMN_CATALOG,cur_row_id_value,dir_catalog + "/");			//add current row_id column to davisbase_columns
		}
		catch (Exception e) {
			System.out.println(e);
		}
    ...
    public static void parseDeleteString(String deleteString) {
		...
		String rawCondition = deleteString.split("where")[1];
		String[] parsedCondition = parseCondition(rawCondition);
		if(tableExists(table)){
			Table.delete(table, parsedCondition);
		}
		else
		{
			System.out.println("Table "+table+" does not exist.");
    ...
    public static void parseUpdateString(String updateString) {
		...
		if(!tableExists(table)){
			Table.update(table, parsedCondition, parsedSetCondition, dir_userdata);
		}
		else
		{
			System.out.println("Table "+table+" does not exist.");
